...
function DailyBonusConfig() {
  ...
  const geCampaigns = async () => {
    setLoading(true)
    const res: any = await dailyBonusConfigApi.getDailyBonuses(currentPage)
  ...
  return (
    <div>
      <div className='mb-4 flex items-center justify-between'>
        <span className='text-title-md2 font-semibold text-black'>Daily Bonus Config</span>
        <ButtonWithIcon name='Add New' className='mb-5 mt-5' path={`add`} />
      </div>

      <TableCampaigns
        list={dailyBonus.dailyBonus}
        //todo: Change page
        total={5}
...
const TableCampaigns = (props: any) => {
    ...
    <div className='rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1'>
      <HelmetProvider>
        <div>
          <Helmet>
            <meta charSet='utf-8' />
            <title>List Daily Bonus Config</title>
        ...
        <table className='w-full table-auto'>
          <thead>
            <tr className='bg-gray-2 text-left dark:bg-meta-4'>
              <th className='min-w-[220px] py-4 px-4 font-semibold text-black dark:text-white xl:pl-11'>Campaign Id</th>
